...
Then we are checking If all of array's elements have `URL` and `name` properties. And also They have been checked for being an empty string.
...
Then we are testing If slider menu show/hide functionality is working properly. To do this we are selecting element which has class `.menu-icon-link` and testing clicks on It. In every single click `.menu-hidden` class should removed and re-implement to achieve a proper show/hide action.
